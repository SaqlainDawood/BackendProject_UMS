...
const sendEmail = async ({ to, subject, html, text }) => {
  try {
    ...
    console.log(`✅ Staff email sent to ${to} — msgId: ${res.data?.messageId}`);
    return { success: true, messageId: res.data?.messageId };
  } catch (err) {
    const msg =
      err.response?.data?.message || err.response?.data?.error || err.message;
    console.error("❌ staffEmailService error:", msg);
...
/* ============================================================
   ...
   ============================================================ */
export const sendStaffVerificationEmail = async ({ to, name, verifyUrl }) => {
  const html = baseTemplate({
    title: "Verify Your Email Address",
    bodyHtml: `
      <p>Hi <strong>${name || "Applicant"}</strong>,</p>
      <p>Thank you for signing up as a staff applicant. Please verify your email address to continue your application.</p>
...
/* ============================================================
   ...
   ============================================================ */
export const sendStaffWelcomeEmail = async ({ to, name }) => {
  const html = baseTemplate({
    title: `Welcome, ${name || "Applicant"}! 🎉`,
    bodyHtml: `
      <p>Your email has been successfully verified.</p>
      <p>You can now continue filling your staff application. Login with your credentials to proceed.</p>
      <p>Complete all 6 steps and submit your application for review.</p>
    `,
...
/* ============================================================
   ...
   ============================================================ */
export const sendStaffApplicationSubmittedEmail = async ({ to, name }) => {
  const html = baseTemplate({
    title: "Application Submitted Successfully ✅",
    bodyHtml: `
      <p>Hi <strong>${name || "Applicant"}</strong>,</p>
      <p>Your staff application has been submitted successfully. Our HR team will review it shortly.</p>
      ...
      <p>You'll receive an email update on each step.</p>
    `,
    actionUrl: process.env.FRONT_END_URL,
...
/* ============================================================
   4. APPLICATION APPROVED — Full Version
   ============================================================ */
export const sendStaffApprovedEmail = async ({
  ...
  const appName = process.env.UNIVERSITY_NAME || "UMS Portal";
  const loginUrl =
    process.env.FRONT_END_URL || "https://studentteacherportal.vercel.app";
  const contact = process.env.UNIVERSITY_CONTACT || "";
      ...
      <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.06);">
        ...
        <tr>
          <td style="padding:40px 32px;">
            ...
            <div style="background:#f0fdf4;border-left:4px solid #16a34a;padding:20px;margin:0 0 24px;border-radius:8px;">
              <p style="margin:0 0 12px;font-weight:700;color:#166534;font-size:15px;">
                🔐 Your Login Credentials
              </p>
              <table style="width:100%;font-size:14px;color:#374151;">
                <tr><td style="padding:6px 0;width:130px;"><strong>Email:</strong></td>
                    <td style="padding:6px 0;">${to}</td></tr>
                <tr><td style="padding:6px 0;"><strong>Password:</strong></td>
                    <td style="padding:6px 0;">
                      <code style="background:#fff;padding:4px 10px;border-radius:4px;font-family:monospace;border:1px solid #d1fae5;color:#166534;font-weight:600;">${tempPassword}</code>
                    </td></tr>
                <tr><td style="padding:6px 0;"><strong>Role:</strong></td>
                ...
                ${designation ? `<tr><td style="padding:6px 0;"><strong>Designation:</strong></td><td style="padding:6px 0;">${designation}</td></tr>` : ""}
                ${joiningDate ? `<tr><td style="padding:6px 0;"><strong>Joining Date:</strong></td><td style="padding:6px 0;">${joiningDate}</td></tr>` : ""}
              </table>
            </div>

            <!-- SECURITY WARNING -->
            <div style="background:#fef2f2;border-left:4px solid #dc2626;padding:14px 16px;margin:0 0 24px;border-radius:6px;">
              <p style="margin:0;color:#991b1b;font-size:13px;line-height:1.5;">
                ⚠️ <strong>Security Note:</strong> Please change this password immediately after your first login.
              </p>
            </div>

            <!-- CTA BUTTON -->
            <div style="text-align:center;margin:32px 0;">
              <a href="${loginUrl}" style="display:inline-block;background:#16a34a;color:#ffffff;text-decoration:none;padding:14px 40px;border-radius:8px;font-weight:600;font-size:15px;">🚀 Login to Your Account</a>
            </div>
            ...
            <div style="border-top:1px solid #e5e7eb;padding-top:24px;margin-top:32px;">
              ...
              <p style="color:#374151;font-size:15px;line-height:1.7;margin:0 0 12px;">
                We're thrilled to have you join our team. Your skills and experience will be a valuable addition to ${appName}. We look forward to working together and achieving great things.
              </p>
              <p style="color:#374151;font-size:15px;line-height:1.7;margin:0;">
                If you have any questions, feel free to reach out to us${contact ? ` at <strong>${contact}</strong>` : ""}.
              </p>
            </div>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#f9fafb;padding:24px 32px;text-align:center;border-top:1px solid #e5e7eb;">
            <p style="margin:0 0 4px;color:#6b7280;font-size:14px;font-weight:600;">${appName}</p>
            <p style="margin:0;color:#9ca3af;font-size:12px;line-height:1.5;">
              This is an automated message. Please do not reply.
              ${contact ? `<br/>Contact: ${contact}` : ""}
            </p>
          </td>
        </tr>
...
/* ============================================================
   5. APPLICATION REJECTED — Full Version
   ============================================================ */
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f6f8;padding:40px 0;">
    <tr><td align="center">
      <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.06);">
        ...
        <!-- HEADER -->
        <tr>
          <td style="background:linear-gradient(135deg,#475569,#64748b);padding:40px 32px;text-align:center;">
        ...
        <tr>
          <td style="padding:40px 32px;">
            ...
            <p style="color:#374151;font-size:15px;line-height:1.7;margin:0 0 20px;">
              ...
              ${role ? `<strong>${role}</strong>` : ""} at ${appName}.
              We truly appreciate your interest in joining our team.
            </p>

            <p style="color:#374151;font-size:15px;line-height:1.7;margin:0 0 20px;">
              After careful consideration of all applications, we regret to
              inform you that your application has
              <strong style="color:#dc2626;">not been shortlisted</strong>
              at this time.
            </p>
            ...
            ${
              reason
                ? `
            <!-- REASON BOX -->
            <div style="background:#fef2f2;border-left:4px solid #dc2626;padding:16px 20px;margin:0 0 24px;border-radius:8px;">
              <p style="margin:0 0 6px;font-weight:700;color:#991b1b;font-size:14px;">
            ...
            <!-- TIPS BOX -->
            <div style="background:#f1f5f9;border-left:4px solid #475569;padding:16px 20px;margin:0 0 24px;border-radius:8px;">
              <p style="margin:0 0 10px;font-weight:700;color:#334155;font-size:14px;">
...
export const sendStaffPasswordResetEmail = async ({
  ...
  const html = baseTemplate({
    ...
    bodyHtml: `
      <p>Hi <strong>${name || "Applicant"}</strong>,</p>
      <p>We received a request to reset your staff account password. Click the button below to set a new password.</p>
  ...
  const html = baseTemplate({
    ...
    bodyHtml: `
      ...
      <p style="margin-top:16px;">
        <strong>Time:</strong> ${time || new Date().toLocaleString()}
      </p>
      <p style="color:#dc2626;font-weight:500;margin-top:16px;">

...
export const createBatchSemesterSubject = async (req, res) => {
  try {
    const { batchId, semester, subjectId, teacherId, isActive } = req.body;
    if (!batchId || !semester || !subjectId || !teacherId) {
      return res.status(400).json({
        success: false,
        message: "batchId, semester, subjectId and teacherId are required",
      });
    }
    ...
    if (batch.totalSemesters && semesterNumber > batch.totalSemesters) {
      return res.status(400).json({
        success: false,
        message: `This batch only has ${batch.totalSemesters} semesters`,
      });
    }

    if (subject.semester && Number(subject.semester) !== semesterNumber) {
      return res.status(400).json({
        success: false,
        message:
    ...
    if (String(batch.degreeClassId) !== String(subject.degreeClassId)) {
      return res.status(400).json({
        success: false,
        message:
    ...
    let batchSemesterSubject = await BatchSemesterSubject.findOne({
      batchId,
      semester: semesterNumber,
      subjectId,
    });
    if (!batchSemesterSubject) {
      batchSemesterSubject = await BatchSemesterSubject.create({
        batchId,
        semester: semesterNumber,
        subjectId,
    ...
    const existingTeacherSubject = await TeacherSubject.findOne({
      teacherId,
      batchSemesterSubjectId: batchSemesterSubject._id,
    });
    ...
    const populated = await populateTeacherSubject(
      TeacherSubject.findById(teacherSubject._id),
    );
    ...
    console.error("Create Batch Semester Subject Error:", err);
...
export const getBatchSemesterSubjects = async (req, res) => {
  try {
    const { batchId, semester, subjectId, isActive } = req.query;
    ...
    const assignments = await populateBatchSemesterSubject(
      BatchSemesterSubject.find(filter).sort({
        semester: 1,
        createdAt: -1,
      }),
    );
    ...
    console.error("Get Batch Semester Subjects Error:", err);
...
export const getSubjectsByBatchSemester = async (req, res) => {
  try {
...
export const getBatchSemesterSubjectById = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const assignment = await populateBatchSemesterSubject(
    ...
    const teachers = await populateTeacherSubject(
      TeacherSubject.find({
    ...
    console.error("Get Batch Semester Subject Error:", err);
    return res.status(400).json({
      success: false,
      message: cleanErrorMessage(err),
    });
...
export const updateBatchSemesterSubject = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const assignment = await BatchSemesterSubject.findById(id);
    ...
    const updated = await populateBatchSemesterSubject(
      BatchSemesterSubject.findById(id),
    );
    return res.status(200).json({
      success: true,
      message: "Batch semester subject updated successfully",
      data: updated,
    });
  } catch (err) {
    console.error("Update Batch Semester Subject Error:", err);
    return res.status(400).json({
      success: false,
      message: cleanErrorMessage(err),
    });
...
export const deleteBatchSemesterSubject = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const assignment = await BatchSemesterSubject.findByIdAndDelete(id);
    if (!assignment) {
      return res.status(404).json({
        success: false,
        message: "Batch semester subject not found",
    ...
    console.error("Delete Batch Semester Subject Error:", err);
...
export const getTeacherAssignedSubjects = async (req, res) => {
  try {
    ...
    if (semester !== undefined) {
      ...
      const teacherAssignments = await populateTeacherSubject(
        TeacherSubject.find({
          ...filter,
      ...
      const filtered = teacherAssignments.filter(
        (item) => item.batchSemesterSubjectId?.semester === semesterNumber,
      );
    ...
    const assignments = await populateTeacherSubject(
      TeacherSubject.find(filter).sort({
        createdAt: -1,
      }),
    );
...
export const updateTeacherSubjectAssignment = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (teacherId !== undefined) {
      ...
      if (teacher.isActive === false) {
        return res.status(400).json({
          success: false,
          message: "This teacher is inactive",
        });
      }

      const duplicate = await TeacherSubject.findOne({
        teacherId,
        batchSemesterSubjectId: assignment.batchSemesterSubjectId,
        _id: {
          $ne: id,
        },
      });
    ...
    const updated = await populateTeacherSubject(TeacherSubject.findById(id));
    ...
    console.error("Update Teacher Subject Assignment Error:", err);
...
export const deleteTeacherSubjectAssignment = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error("Delete Teacher Subject Assignment Error:", err);
